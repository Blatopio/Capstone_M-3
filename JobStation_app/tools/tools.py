...
def get_sql_database() -> SQLDatabase:
    """
    Returns a read-only SQLDatabase instance connected to candidates table.
    Connection string format: mysql+mysqlconnector://user:pass@host:port/db
    """
...
def get_sql_agent():
    """
    Returns a SQL agent that can only SELECT from candidates table.
    System prompt enforces state='available' filter always.
    """
    db = get_sql_database()
...
- id          : candidate ID (INT)
- category    : job category e.g. HR, ENGINEERING, FINANCE (VARCHAR)
...
def search_candidates_tool(query: str, category: str, prof_level: str, top_k: int = 3) -> str:
    ...
    Step 1 — SQL Agent: generates and runs a SELECT query to filter
             candidates by category, prof_level, state='available'
    ...
    """
    # Step 1 — SQL Agent generates and runs the query
    ...
    # Step 2 — Qdrant semantic search with metadata filter
